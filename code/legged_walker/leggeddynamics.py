...
class kneedWalker(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('cost', CostFunc(num_nodes=nn, states_ref=self.options['states_ref'] ), promotes_inputs=['q1', 'q2', 'tau'], promotes_outputs=['*'])
...
class CostFunc(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials(of=['costrate'], wrt=['q1', 'q2', 'tau',], method='exact', rows=np.arange(nn), cols=np.arange(nn))
